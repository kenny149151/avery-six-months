(() => {
  ...
  function renderHistory() { const list = document.querySelector('#history-list'); document.querySelector('#history-count').textContent = records.length; document.querySelector('#clear-records').disabled = records.length === 0; if (!records.length) { list.innerHTML = '<p class="empty-history">保存今天的记录后，这里会自动留下每天的小足迹。记录保存在这台手机的浏览器里。</p>'; return; } list.innerHTML = `<div class="history-list">${records.slice(0, 7).map((item) => { const date = new Date(item.createdAt); const count = item.completedTasks.filter(Boolean).length; const reportUrl = `./report.html?data=${encodeURIComponent(JSON.stringify(item))}`; return `<div class="history-row"><div class="history-date"><strong>${date.toLocaleDateString('zh-CN', { month: 'numeric', day: 'numeric' })}</strong><small>${date.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}</small></div><div class="history-bar"><span style="width:${Math.max(18, count / 3 * 100)}%"></span><small>${count}/3 项完成</small></div><div class="history-water">💧 ${item.waterMl} ml</div><a href="${reportUrl}" target="_blank" rel="noreferrer">查看 ↗</a></div>`; }).join('')}</div>`; }
  function saveRecord() { const item = todayRecord(); records.unshift(item); records = records.slice(0, 30); persist(); renderHistory(); const report = `Avery 今日喂养报告：已完成 ${done.filter(Boolean).length}/3 项，记录饮水 ${water} ml。`; const url = `${location.origin}${location.pathname}report.html?data=${encodeURIComponent(JSON.stringify(item))}`; document.querySelector('#record-result').innerHTML = '今天已保存 ✓<br><span>详细报告已生成，可打开或发给爸妈。</span>'; const reportButton = document.querySelector('#report-button'); reportButton.href = url; reportButton.hidden = false; const sms = document.querySelector('#sms-button'); sms.hidden = false; sms.dataset.report = `${report} 打开详细报告：${url}`; }
  function clearAllRecords() { if (!records.length || !window.confirm('确定要清空 Avery 的全部记录吗？清空后无法恢复。')) return; records = []; persist(); done = [false, false, false]; water = 0; document.querySelector('#record-result').textContent = ''; document.querySelector('#report-button').hidden = true; document.querySelector('#sms-button').hidden = true; updateProgress(); updateWater(); renderHistory(); }
  ...
  document.querySelector('#clear-records').addEventListener('click', clearAllRecords);
